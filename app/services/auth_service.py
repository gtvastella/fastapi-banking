...
class AuthService:
    ...
    def register_user(self, user_data: PersonCreate) -> Dict[str, Any]:
        if user_data.person_type == TYPE_NATURAL_PERSON:
            ...
            raise ValidationException(message="Tipo de pessoa inválido", error_code="INVALID_PERSON_TYPE")
    ...
    def login_user(self, login_data: LoginRequest) -> Tuple[str, Dict[str, Any]]:
        try:
            ...
            if not user or not verify_password(login_data.password, user.password):
                raise UnauthorizedException(message="Email ou senha incorretos", error_code="INVALID_CREDENTIALS")
    ...
    def _check_email_availability(self, email: str) -> None:
        ...
        if existing_user:
            raise BadRequestException(message="Email já cadastrado", error_code="EMAIL_EXISTS")
